...
namespace MultilanguageChat.ViewModels
{
    public class MainViewModel : ViewModelBase
    {
        ...
        {
            translatorClient = SimpleIoc.Default.GetInstance<ITranslatorClient>();
            speechClient = SimpleIoc.Default.GetInstance<ISpeechClient>();
            audioService = SimpleIoc.Default.GetInstance<IAudioService>();

            messageService = SimpleIoc.Default.GetInstance<IMessageService>();
            messageService.OnMessageReceived(OnMessageReceived);

            UserName = Xamarin.Essentials.DeviceInfo.Name;
        ...
        private async void OnMessageReceived(ChatMessage message)
        {
            try
            {
                ...
                await CoreApplication.MainView.CoreWindow.Dispatcher.RunAsync(CoreDispatcherPriority.Normal, () => Messages.Add(message));
